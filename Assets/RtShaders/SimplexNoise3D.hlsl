...
float snoise(float3 v)
{
    ...
    // Permutations
    i = mod2893D(i); // Avoid truncation effects in permutation
    float4 p =
      permute4D(permute4D(permute4D(i.z + float4(0.0, i1.z, i2.z, 1.0))
    ...
    // Normalise gradients
    float4 norm = taylorInvSqrt4D(float4(dot(g0, g0), dot(g1, g1), dot(g2, g2), dot(g3, g3)));
    g0 *= norm.x;
    g1 *= norm.y;
    g2 *= norm.z;
    g3 *= norm.w;
