...
float cnoise(float3 P)
{
  ...
  float4 ixy = permute4D(permute4D(ix) + iy);
  float4 ixy0 = permute4D(ixy + iz0);
  float4 ixy1 = permute4D(ixy + iz1);
  ...
  float4 norm0 = taylorInvSqrt4D(float4(dot(g000, g000), dot(g010, g010), dot(g100, g100), dot(g110, g110)));
  g000 *= norm0.x;
  g010 *= norm0.y;
  g100 *= norm0.z;
  g110 *= norm0.w;

  float4 norm1 = taylorInvSqrt4D(float4(dot(g001, g001), dot(g011, g011), dot(g101, g101), dot(g111, g111)));
  g001 *= norm1.x;
  g011 *= norm1.y;
  g101 *= norm1.z;
  g111 *= norm1.w;
